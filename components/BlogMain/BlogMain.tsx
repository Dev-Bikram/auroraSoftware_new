...
export default function BlogMain() {
    ...
    const { data: allBlogs, isLoading } = useQuery('allBlogs', fetchBlogList);
        ...
        <Container fixed>
            <Box className="mainblog-listwrap">
                <Grid container spacing={{ xs: 3, md: 4 }} columns={{ xs: 4, sm: 8, md: 12 }}>
                    {allBlogs?.data?.data?.map((allblog : any) => (
                        <Grid item xs={8} sm={4} md={3} key={allblog._id}>
                            <Box className="singleblogWrap">
                                <Box className="blog-tittleWrap">
                                    <Typography variant='h1'>{allblog.title}</Typography>
                                </Box>
                                {/* <figure>
                                ...
                                <Box className="blogtxtwrap">
                                    <Typography variant='caption'>{allblog.title}</Typography>
                                    <Typography variant='body1'>
                                        <Link href={`/blog/${allblog._id}`}>{allblog.status}</Link>
                                    </Typography>
                                    {/* <Link className="blogbtnwrap" href='/blog-details'><Image src={allblog.image} width={18} height={9} alt='bloglink-image' /></Link> */}
                                </Box>
                            </Box>
                        </Grid>
                    ))}
